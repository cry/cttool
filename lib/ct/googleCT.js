...
const parseRes = (data, certs = []) => {
		...
		log.info("Recieved results from #%d.", data.startIndex);
	...
	return searchAllTLS({
		...
		.then((data) => {
			return new Promise((resolve, reject) => {
				...
				data.forEach((el, ind) => {
					var nonWildcard = el.subject.replace("*.", "");

					if (!~uniqSubjects.indexOf(nonWildcard)) uniqSubjects.push(nonWildcard);
				...
				Promise.all(promises)
					.then(() => {
						log.info(util.format("Found %d unique hostnames in CT log.", uniqSubjects.length));
